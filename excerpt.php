...
while ( have_posts() ) : the_post(); 
    ...
    echo '<article class="excerpt excerpt-'.$ii. $_excerpt_text .'">';
        ...
        if( _hui('post_plugin_date') ){
            echo '<time><i class="fa fa-clock-o"></i>'. _set_time_ago(get_the_time('Y-m-d G:i:s')).'</time>';
            // echo '<abbr title="' . the_time() . '">' . apply_filters( 'post_date_column_time', the_time(), $post, 'date' ) . '</abbr>';
        }
        ...
        if( _hui('post_plugin_view') ){
            echo '<span class="pv">'.$fire.'&nbsp;'._get_post_views().'</span>';
        }
